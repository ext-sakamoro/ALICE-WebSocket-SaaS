...
async fn main() {
    ...
    let state = Arc::new(AppState {
        core_url: env("CORE_ENGINE_URL", "http://core-engine:8131"),
    ...
    let app = Router::new()
        ...
        .with_state(state);
    let addr = std::env::var("GATEWAY_ADDR").unwrap_or_else(|_| "0.0.0.0:8130".into());
    let listener = tokio::net::TcpListener::bind(&addr).await.unwrap();
    tracing::info!("WebSocket API Gateway on {addr}");
...
async fn license_handler() -> (HeaderMap, Json<LicenseInfo>) {
    ...
    (h, Json(LicenseInfo {
        ...
        source_code: "https://github.com/ext-sakamoro/ALICE-WebSocket-SaaS".into(),
